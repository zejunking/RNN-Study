...
class HuXiu(object):
    ...
    def visit_index(self, tmp):
        ...
        for i in range(4):
            # 进入模拟拖动流程
            self.analog_drag(tmp)
        # self.driver.close()

    def analog_drag(self,tmp):
        # 鼠标移动到拖动按钮，显示出拖动图片
        ...
        # 保存图片方便查看
        cut_image.save("cut{tmp}{distance}.jpg".format(tmp=tmp,distance=distance))
        full_image.save("full{tmp}{distance}.jpg".format(tmp=tmp,distance=distance))
