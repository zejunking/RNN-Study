...
def saveSql(distance, sign, index):
    pwd = os.path.abspath('.')
    cur = os.path.join(pwd, 'sliceTestImage/{distance}'.format(distance=distance))
    if os.path.exists(cur) is False:
        os.makedirs(cur)

    resultPath = 'sliceTestImage/{distance}/test{uid}.png'.format(uid=index, distance=distance)
    print(resultPath)
    img = cv2.imread(path, 0)  # 原始图像
    cv2.imwrite(resultPath, img)
    curTime = int(time.time())
    cursor.execute('INSERT INTO TAB_SLICE_IMG_TEST_INFO (imgUrl, imgNum, createdTime, sign) VALUES ("test{uid}.png", {distance}, {curTime}, {sign})'.format(uid=index, distance=distance, curTime=curTime, sign=sign))
    db.commit()
